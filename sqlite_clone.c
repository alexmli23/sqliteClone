...
const uint32_t INTERNAL_NODE_KEY_SIZE = sizeof(int32_t);
 ...
 void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num)
{
    ...
    *internal_node_right_child(old_node) = *internal_node_child(old_node, *old_num_keys - 1);
    ...
    uint32_t max_after_spllit = get_node_max_key(table->pager, old_node);
    uint32_t destination_page_num = child_max < max_after_spllit ? old_page_num : new_page_num;
